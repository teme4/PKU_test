...
void lcd:: PulseLCD()
{  
    STM32F407.set_pin_state(GPIOD,gpio_lcds.EN,0);
    delay_us(1);
    STM32F407.set_pin_state(GPIOD,gpio_lcds.EN,1);
    delay_us(1);
    STM32F407.set_pin_state(GPIOD,gpio_lcds.EN,0);
    delay_us(1);
} 
...
void lcd::SendByte(char ByteToSend, int IsData)
{
    ...
    if (IsData == 1)
        STM32F407.set_pin_state(GPIOD,gpio_lcds.RS,1);
    else
     STM32F407.set_pin_state(GPIOD,gpio_lcds.RS,0);
     PulseLCD();
     GPIOD->ODR &=~0x3c0;
...
void lcd:: busy_flag()
{
    STM32F407.gpio_init(GPIOD,gpio_lcds.data_7,gpio_type::gpio_type_pp,gpio_mode::gpio_mode_input,gpio_speed::gpio_speed_very_high);  
    while(1)
    {
    STM32F407.set_pin_state(GPIOD,gpio_lcds.RS,0);
    STM32F407.set_pin_state(GPIOD,gpio_lcds.RW,1); 
    _temp=STM32F407.get_state_pin(GPIOD,gpio_lcds.data_7); 
    if(_temp==0)
    {
        break;
    }
    STM32F407.set_pin_state(GPIOD,gpio_lcds.RS,0);
    STM32F407.set_pin_state(GPIOD,gpio_lcds.RW,0);
    }   
